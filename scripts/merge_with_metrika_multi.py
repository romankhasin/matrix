#!/usr/bin/env python3
"""
Add Metrika postclick columns (bounce_rate, avg_time_on_site_sec,
metrika_visits) to data/merged_<month>.csv, matched by canonical platform.
Generalized version of merge_with_metrika.py for an arbitrary month.

Usage:
    python3 merge_with_metrika_multi.py --month may2026
"""
import argparse
import csv

BASE = "/home/claude/matrix/data"


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--month", required=True, help="e.g. may2026")
    args = parser.parse_args()

    metrika = {}
    with open(f"{BASE}/metrika_{args.month}.csv", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            metrika[row["platform"]] = row

    merged_path = f"{BASE}/merged_{args.month}.csv"
    with open(merged_path, encoding="utf-8") as f:
        reader = csv.DictReader(f)
        rows = list(reader)
        base_fields = [fn for fn in reader.fieldnames if fn not in ("bounce_rate", "avg_time_on_site_sec", "metrika_visits")]

    new_fields = base_fields + ["bounce_rate", "avg_time_on_site_sec", "metrika_visits"]
    matched = 0
    for r in rows:
        m = metrika.get(r["platform"])
        if m:
            r["bounce_rate"] = m["bounce_rate"]
            r["avg_time_on_site_sec"] = m["avg_time_on_site_sec"]
            r["metrika_visits"] = m["visits"]
            matched += 1
        else:
            r["bounce_rate"] = ""
            r["avg_time_on_site_sec"] = ""
            r["metrika_visits"] = ""

    with open(merged_path, "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=new_fields)
        w.writeheader()
        w.writerows(rows)

    print(f"[{args.month}] matched {matched}/{len(rows)} platforms with metrika postclick data -> {merged_path}")


if __name__ == "__main__":
    main()
